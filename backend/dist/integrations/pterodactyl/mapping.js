...
export function resolveEggTemplate(game, edition) {
    const template = EGG_TEMPLATE_MAP.find((entry) => entry.game === game.toLowerCase() && entry.edition === edition.toLowerCase());
    if (!template) {
        throw new Error(`No egg/template mapping found for ${game}:${edition}`);
    }
    return template;
}
export function resolveResourceLimits(plan, boosts = {}) {
    return {
